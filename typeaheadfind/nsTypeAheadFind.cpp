                         ...
                         const PRUnichar *aData)
{
  ...
  if (isOpening) {
    ...
    if (privateWindow) {
      ...
      nsCOMPtr<nsIController> controller = new nsTypeAheadController();
  ...
  fm->GetFocusedWindow(getter_AddRefs(activeWindow));
...
nsTypeAheadFind::HandleChar(PRUnichar aChar)
{
  ...
  if (bufferLength == 0) {
    ...
    if (mIsFirstVisiblePreferred) {
      ...
      nsCOMPtr<nsIDocument> document = do_QueryInterface(presShell->GetDocument());
      if (!document)
  ...
  while (PR_TRUE) {    // ----- Outer while loop: go through all docs -----
    while (PR_TRUE) {  // === Inner while loop: go through a single doc ===
     ...
     if (!mFocusedDocSelection || !mFocusedDocSelCon) {
      ...
      nsCOMPtr<nsIDOMElement> focusedElement;
      fm->MoveFocus(window, nsnull, nsIFocusManager::MOVEFOCUS_CARET,
...
nsTypeAheadController::DoCommand(const char *aCommand)
{
  ...
  nsCOMPtr<nsIDOMWindowInternal> domWinInternal = do_QueryInterface(focusedWindow);
